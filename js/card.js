...
(function () {
  ...
  var fillCards = function (card) {
    ...
    mapCardFeatures.insertAdjacentHTML('afterbegin', card.offer.features.map(getFeatures).join(' '));
    mapCard.appendChild(mapCardFeatures);
    mapCardPictures.innerHTML = '';
    mapCardPictures.insertAdjacentHTML('afterbegin', card.offer.photos.map(getPhotos).join(''));
